...
def Calculations(time, reflectWave, transWave, K, barWaveSpeed, 
                 barElasticModulus, barDiameter, specimenLength, 
                 specimenDiameter):
    #计算应力、应变、应变率
    ...
    engStress = [barElasticModulus * (barDiameter / specimenDiameter)**2 * 
              np.pi / 4 * temp for temp in transStrain]
...
def FittingStrainRate(dataStoreDir,time, strain):
    ...
    plt.figure(figsize = (8, 4))
    plt.plot(time, strain, "r_", linewidth = 2)
    plt.plot(X, YFitting, "b--", linewidth = 2)
    plt.xlabel("Time/$s$")
    plt.ylabel("Strain")
